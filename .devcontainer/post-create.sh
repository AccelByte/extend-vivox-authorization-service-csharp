...
if [ -f "src/extend-vivox-authentication-service.sln" ]; then
    dotnet restore src/extend-vivox-authentication-service.sln
else
...
echo "  • Build .NET solution: dotnet build src/extend-vivox-authentication-service.sln"
echo "  • Run .NET service: cd src/AccelByte.Extend.Vivox.Authentication.Server && dotnet run"
